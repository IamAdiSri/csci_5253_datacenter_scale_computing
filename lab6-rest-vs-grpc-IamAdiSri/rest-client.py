...
def doDotProduct(addr, debug=False):
    headers = {'content-type': 'application/json'}
    dp_url = addr + "/api/dotproduct"
    v1 = [random.random() for _ in range(100)]
    v2 = [random.random() for _ in range(100)]
    data = {'v1': v1, 'v2': v2}
    response = requests.post(dp_url, data=json.dumps(data), headers=headers)
    if debug:
        # decode response
        print("Response is", response)
        print(json.loads(response.text))
...
def doJsonImage(addr, debug=False):
    ...
    response = requests.post(image_url, data=json.dumps(data), headers=headers)
    if debug:
        # decode response
        print("Response is", response)
        print(json.loads(response.text))
